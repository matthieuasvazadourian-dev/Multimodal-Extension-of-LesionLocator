    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                # Under intermediate fusion the weight-tied encoder runs once per present
                # modality per forward (see EmbeddingExtractor docstring); tell both
                # extractors how to label those multiple invocations for THIS case.
                # None (non-fusion, or an unresolved case) leaves layers that only ever
                # fire once completely unaffected.
                if self.intermediate_fusion_mode:
                    if case_inference_modality == 'ct':
                        case_pass_labels = ['ct']
                    elif case_inference_modality == 'pet':
                        case_pass_labels = ['pet']
                    else:
                        case_pass_labels = ['ct', 'pet']
                else:
                    case_pass_labels = None
                if self.embedding_extractor is not None:
