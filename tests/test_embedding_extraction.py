...
tests/test_ct_passthrough.py / tests/test_fusion_stability.py (no GPU
required).
...
def test_encoder_passes_not_overwritten(fusion_arch):
    ...
    extractor.pass_labels = ['ct', 'pet']  # what the predictor sets for a two-modality case
...
def test_forced_single_modality_labels_encoder_but_not_fusion_or_decoder():
    """Under missing_modality_robust with a modality forced absent, only one
    encoder pass runs. It should still be labeled by modality (so the .npz
    self-documents which one), but fusion_modules/decoder outputs -- which
    always fire exactly once no matter how many modalities were present --
    must stay bare even though pass_labels happens to have length 1."""
    torch.manual_seed(0)
    model = _build_fusion('weighted', missing_modality_robust=True).eval()
    model.inference_modality = 'ct'
    extractor = EmbeddingExtractor(model, layer_names=['decoder.stages.4', 'encoder.stages.0'])
    extractor.pass_labels = ['ct']  # what the predictor sets when case_inference_modality == 'ct'
    ...
    assert 'encoder.stages.0_ct' in emb, emb.keys()
    assert 'encoder.stages.0' not in emb
    assert 'encoder.stages.0_pet' not in emb

    assert 'decoder.stages.4' in emb
    fusion_keys = [k for k in emb if k.startswith('fusion_modules.')]
    assert fusion_keys
    assert all(not (k.endswith('_ct') or k.endswith('_pet')) for k in fusion_keys)
